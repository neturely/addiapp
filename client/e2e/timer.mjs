...
import { launch, login, seedTask, reporter, sleep, waitForCompletion, BASE } from './lib.mjs'
...
ok(
  (await page.$('header a[href^="/play/progress/"]')) === null,
  '#135: no chip when nothing is in progress',
)
...
await page.goto(`${BASE}/dashboard`, { waitUntil: 'networkidle0' })
ok(
  (await page.$(CHIP(id))) !== null,
  '#135: chip appears for the in-progress task, linking to /play/progress/:id',
)
...
ok(
  (await page.$(CHIP(id))) === null,
  '#419: chip hides while its task is the viewed InProgress screen',
)
...
await page.goto(`${BASE}/play/progress/${id}`, { waitUntil: 'networkidle0' })
ok(
  (await page.$(CHIP(id2))) !== null,
  '#419: chip shows the OTHER running task on an InProgress screen',
)
...
ok(
  (await page.$('header a[href^="/play/progress/"]')) === null,
  '#419: chip hides while the column mirror is visible',
)
...
ok(
  wideRow.dot && !wideRow.digits,
  '#419: Started row keeps the pulse dot but drops the ticking digits (wide)',
)
...
// scoring-independent: this suite is about the chip, not the award (#437)
await waitForCompletion(page)
await sleep(400)
await page.goto(`${BASE}/settings`, { waitUntil: 'networkidle0' })
ok(
  (await page.$(CHIP(id))) === null,
  '#135: chip disappears after in-place completion (imperative refresh)',
)
